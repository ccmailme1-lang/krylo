...
// Locked Fs weights — DO NOT MODIFY without formal WO + SAB notification
const FS_WEIGHTS = {
  m_checksum:  0.40,
  t_telemetry: 0.30,
  d_docs:      0.20,
...
function mapPosture(fs) {
  ...
  return 'CALM';
  // NOTE: Fs < 0.40 withdrawal/fading behavior BLOCKED — SAB review pending
}

function enrichRecord(record) {
  validateRecord(record);
  const fs = calculateFs(record.fidelity_components);s
  return {
    ...record,
    fs,
    posture: mapPosture(fs),
  };
}
