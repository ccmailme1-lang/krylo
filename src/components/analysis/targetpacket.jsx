        ...
        <div style={{
          ...
          <>
              ...
              <div ref={actionPanelRef} onScroll={checkActionScroll} style={{ flex: 1, minHeight: 0, padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: 12, overflowY: 'auto' }}>
                ...
                {/* ── ACTION PLAN — WO-1822 / WO-1820 / WO-1821 ─────────── */}
                {hp?.qualified ? (
                  <div style={{ border: `1px solid rgba(138,43,226,0.3)`, background: 'rgba(138,43,226,0.04)', padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <span style={{ fontFamily: MONO, fontSize: 6, color: 'rgba(138,43,226,0.5)', letterSpacing: '0.18em' }}>
                        {hp.domains.join(' · ')}
                      </span>
                    </div>
                    {/* Metrics */}
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, borderTop: `1px solid rgba(138,43,226,0.12)`, paddingTop: 8 }}>
                        ...
                        {alerts.map(a => (
                          <div key={a.id} style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                            <span style={{ fontFamily: MONO, fontSize: 6, color: 'rgba(138,43,226,0.35)', flexShrink: 0 }}>
                              {new Date(a.ts).toTimeString().slice(0, 8)}
                            </span>
                  ...
                  // Silent absence — spec: no proximity indicator when not qualified
                  null
                )}

                {/* ── ACTIVE CONVICTIONS — WO-1823 ────────────────────────── */}
                {convictions.active.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    ...
                    {convictions.active.map(c => {
                      ...
                      const borderColor = highAlerts.length > 0
                        ? 'rgba(255,80,80,0.5)'
                        : mAlerts.some(a => a.severity === 'medium')
                          ? 'rgba(138,43,226,0.6)'
                          : 'rgba(138,43,226,0.4)';
                      return (
                        <div key={c.id} style={{ borderLeft: `2px solid ${borderColor}`, paddingLeft: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                            ...
                            <span style={{ fontFamily: MONO, fontSize: 6, color: 'rgba(138,43,226,0.35)', letterSpacing: '0.12em' }}>
                              {c.domains.join(' · ')}
                            </span>
                          </div>
                          {c.thesis && (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 3, borderTop: `1px solid rgba(255,255,255,0.05)`, paddingTop: 5 }}>
                              {mAlerts.map((a, i) => (
                                <div key={i} style={{ fontFamily: MONO, fontSize: 7, lineHeight: 1.5, letterSpacing: '0.03em',
                                  color: a.severity === 'high'          ? 'rgba(255,80,80,0.85)'
                                       : a.severity === 'medium'        ? 'rgba(255,255,255,0.55)'
                                       :                                   'rgba(102,255,0,0.5)',
                                }}>
                                  {a.message}
                                </div>
                          ...
                          <div style={{ display: 'flex', gap: 6 }}>
                            {['confirmed', 'denied', 'timed_out'].map(res => (
                              <button
                                key={res}
                                onClick={() => convictions.resolve(c.id, res)}
                                style={{
                            ...
                            <button
                              onClick={() => convictions.dismiss(c.id)}
                              style={{
                                fontFamily: MONO, fontSize: 6, letterSpacing: '0.16em', textTransform: 'uppercase',
                                background: 'transparent', cursor: 'pointer', padding: '3px 8px',
                ...
                {convictions.resolved.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8, borderTop: `1px solid rgba(255,255,255,0.06)`, paddingTop: 12 }}>
                    ...
                    {/* Calibration metrics — only when thresholds met */}
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, padding: '8px 12px', borderLeft: `2px solid rgba(102,255,0,0.2)` }}>
                        {calibration.overallAccuracy !== null && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                            ...
                            <span style={{ fontFamily: MONO, fontSize: 9, color: LIME, letterSpacing: '0.08em' }}>
                              {calibration.confirmed}C · {calibration.denied}D · {calibration.timedOut}T
                            </span>
                          </div>
                        )}
                        {calibration.hpAccuracy !== null && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                            ...
                            <span style={{ fontFamily: MONO, fontSize: 9, color: PURPLE, letterSpacing: '0.08em' }}>
                              {Math.round(calibration.hpAccuracy * 100)}%
                            </span>
                          </div>
                        )}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                      {convictions.resolved.map(r => {
                        const resColor = r.resolution === 'confirmed'  ? LIME
                                       : r.resolution === 'denied'      ? 'rgba(255,80,80,0.8)'
                                       :                                   DIM;
                          ...
                          <div key={r.id} style={{ display: 'flex', flexDirection: 'column', gap: 3, paddingBottom: 5, borderBottom: `1px solid rgba(255,255,255,0.04)` }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                              ...
                              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                                {hadHP && (
                                  <span style={{ fontFamily: MONO, fontSize: 5, letterSpacing: '0.18em', color: PURPLE, border: `1px solid rgba(138,43,226,0.3)`, padding: '1px 4px' }}>HP</span>
                                )}
                                <span style={{ fontFamily: MONO, fontSize: 6, letterSpacing: '0.18em', color: resColor, textTransform: 'uppercase' }}>
