...
// KRYL-1293 — EQ_DOMAINS moved here directly: it's CANONICAL_DOMAINS uppercased, not a
// Happy-Path-specific concept, and the real engine no longer defines a domain-scoped
// export (route-based, not domain-based).
import { CANONICAL_DOMAINS } from '../../engine/ontology.js';
const EQ_DOMAINS = CANONICAL_DOMAINS.map(d => d.toUpperCase());
...
export default function EQCanvas({ onCommitThesis, onSetTrigger, isPremium = true, hpOverride }) {
  ...
  // KRYL-1293 — real pipeline. No per-domain state exists in the real model (route-based,
  // not domain-based) -- domainSignals is honestly undefined; hp below is reconstructed
  // from the real {status, route, tiedRoutes} shape, not the old {happyPath} one.
  const hpEngine = useHappyPathEngine();
  const domainSignals = undefined;
  ...
  useEffect(() => {
    ...
    // Build domain points
    const hp  = hpOverride ?? (hpEngine.status === 'ESTABLISHED' ? { qualified: true, route: hpEngine.route, tiedRoutes: hpEngine.tiedRoutes } : null);
  ...
  }, [domainSignals, hpEngine, hovered]);
  ...
  function handleClick(e) {
    ...
    if (idx !== null) onCommitThesis?.({ domain: EQ_DOMAINS[idx], engineState: hpEngine });
  }
  ...
  return (
    <div style={{ background: '#000', fontFamily: MONO, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div style={{ display: 'flex', padding: '4px 0 6px', borderTop: '1px solid rgba(255,255,255,0.03)' }}>
        {EQ_DOMAINS.map(d => {
          const sig   = undefined; // no per-domain state in the real model
      ...
      {hp?.qualified && (
        <div style={{ display: 'flex', flexWrap: 'wrap', columnGap: 24, rowGap: 6, padding: '8px 16px', borderTop: '1px solid rgba(138,43,226,0.15)' }}>
          {/* KRYL-1089 — this qualification is generated by a mock oscillator
              (Math.random()), not a real signal source. */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <span style={{ fontSize: 9, color: 'rgba(138,43,226,0.5)', letterSpacing: '0.2em', textTransform: 'uppercase' }}>Source</span>
            <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.08em' }}>SIMULATED / DEMO DATA</span>
          </div>
          {[
