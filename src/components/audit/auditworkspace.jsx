...
// ── WO-1026: Traceability Chain ───────────────────────────────────────────────
// Renders character-span proof links per ETR truth_statement.
// Data shape: [{ source_doc_id, char_start, char_end, entity_text, confidence }]
function TraceabilityChain({ spans }) {
  if (!spans?.length) {
    return (
      <div style={{ fontSize: '10px', fontFamily: 'IBM Plex Mono, monospace', color: 'rgba(232,244,255,0.18)', letterSpacing: '0.08em' }}>
        no span data — phase b (live ingestion)
      </div>
    );
  }
  return (
      ...
      {spans.map((span, i) => (
        <div key={i} style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
            <span style={{ fontSize: '9px', fontFamily: 'IBM Plex Mono, monospace', color: '#0096FF', letterSpacing: '0.12em' }}>
              {span.entity_text}
            </span>
            <span style={{ fontSize: '9px', fontFamily: 'IBM Plex Mono, monospace', color: 'rgba(232,244,255,0.28)' }}>
              {(span.confidence * 100).toFixed(0)}%
            </span>
          </div>
          <div style={{ display: 'flex', gap: '14px' }}>
            <span style={{ fontSize: '9px', fontFamily: 'IBM Plex Mono, monospace', color: 'rgba(232,244,255,0.22)' }}>
              {span.source_doc_id ?? '—'}
            </span>
            <span style={{ fontSize: '9px', fontFamily: 'IBM Plex Mono, monospace', color: 'rgba(232,244,255,0.18)', letterSpacing: '0.06em' }}>
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* 8. WO-1026: Character-Level Traceability */}
        <div style={{ padding: '22px 24px 32px' }}>
          <div style={{ fontSize: '9px', color: 'rgba(232,244,255,0.22)', letterSpacing: '0.2em', marginBottom: '14px' }}>
            TRACEABILITY CHAIN
          </div>
          <TraceabilityChain spans={record.traceability ?? []} />
        </div>
