...
const CHECKS = [
  ...
  },
  {
    ...
    run: async () => {
      return new Promise((resolve, reject) => {
        const el = document.querySelector('[data-canvas-mounted="true"]');
        if (el) return resolve('Canvas mounted');
        // Session flag set by SignalMap on mount — covers navigating away before running check
        try {
          if (sessionStorage.getItem('krylo_canvas_init') === '1') {
            return resolve('Canvas initialized (not in current view)');
          }
        } catch {}
        // Wait up to 3s for canvas to appear in DOM
        let tries = 0;
        const interval = setInterval(() => {
          const c = document.querySelector('[data-canvas-mounted="true"]');
          if (c) { clearInterval(interval); resolve('Canvas mounted'); }
          try {
            if (sessionStorage.getItem('krylo_canvas_init') === '1') {
              clearInterval(interval); resolve('Canvas initialized (not in current view)');
            }
          } catch {}
          if (++tries > 30) { clearInterval(interval); reject(new Error('Canvas not initialized — navigate to Signal Map first')); }
        }, 100);
      });
    },
  },
  ...
  },
  {
    ...
    run: async () => {
      ...
      if (el) return 'particle-field found';
      // Fallback: session flag set by KineticGravity on first mount.
      // Covers the case where the user is on Signal Map lens (KineticGravity unmounted).
      try {
        if (sessionStorage.getItem('krylo_particle_field_init') === '1') {
          return 'particle-field initialized (not in current view)';
        }
      } catch {}
      throw new Error('.particle-field not found — navigate away from Signal Map first');
    },
  },
  {
    id: 'stubs',
