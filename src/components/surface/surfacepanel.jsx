// surfacepanel.jsx — WO-1718A: Floating collapsible HUD panel
// Tabs: TOWERS / STACK / DELTA. Collapses to tab strip.
import React, { useState } from 'react';
import LeverageTowers from './leveragetowers.jsx';
import { useKalshiSignals } from '../../hooks/usekalshisignals.js';

const LIME  = '#66FF00';
const MONO  = "'IBM Plex Mono', monospace";
const DIM   = 'rgba(255,255,255,0.28)';
const MID   = 'rgba(255,255,255,0.65)';
const TABS  = ['TOWERS', 'STACK', 'DELTA'];

// ── Attention Stack (inline) ─────────────────────────────────────────────────
function AttentionStack({ signals, selectedDomain, onSignalClick }) {
  const seen = new Set();
  const rows = [];
  for (const s of signals) {
    const key = `${s.domain}-${s.signal > 50 ? 'hi' : 'lo'}`;
    if (seen.has(key)) continue;
    seen.add(key);
    rows.push(s);
    if (rows.length >= 6) break;
  }

  if (!rows.length) return (
    <div style={{ padding: '20px 12px', color: DIM, fontSize: 9, letterSpacing: '0.15em' }}>AWAITING SIGNAL</div>
  );

  return (
    <div style={{ padding: '8px 0' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '18px 1fr 32px 32px 36px 28px', padding: '0 12px 4px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        {['#', 'DOMAIN', 'SIG', 'VOL', 'CONF', ''].map(h => (
          <span key={h} style={{ fontSize: 9, letterSpacing: '0.12em', color: DIM }}>{h}</span>
        ))}
      </div>
      {rows.map((s, i) => {
        const isActive = selectedDomain && s.domain?.toUpperCase().includes(selectedDomain.toUpperCase().slice(0, 4));
        return (
          <div
            key={s.ticker ?? i}
            onClick={() => onSignalClick?.(s)}
            style={{
              display: 'grid',
              gridTemplateColumns: '18px 1fr 32px 32px 36px 28px',
              padding: '4px 12px',
              background: isActive ? 'rgba(102,255,0,0.06)' : 'transparent',
              borderLeft: isActive ? `2px solid ${LIME}` : '2px solid transparent',
              cursor: 'pointer',
              transition: 'background 150ms',
            }}
            onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = 'rgba(255,255,255,0.03)'; }}
            onMouseLeave={e => { if (!isActive) e.currentTarget.style.background = 'transparent'; }}
          >
            <span style={{ color: DIM, fontSize: 8 }}>{i + 1}</span>
            <span style={{ color: isActive ? LIME : MID, fontSize: 8, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.domain}</span>
            <span style={{ color: '#fff', fontSize: 9, fontWeight: 600 }}>{s.signal}</span>
            <span style={{ fontSize: 8, color: s.volatility === 'HIGH' ? '#ff4444' : s.volatility === 'MED' ? '#ffaa00' : LIME }}>{s.volatility?.slice(0,3)}</span>
            <span style={{ color: MID, fontSize: 8 }}>{s.confidence}%</span>
            <span style={{ fontSize: 9, color: s.forecast > 2 ? LIME : s.forecast < -2 ? '#ff4444' : DIM }}>
              {s.forecast > 2 ? '↑' : s.forecast < -2 ? '↓' : '—'}
            </span>
          </div>
        );
      })}
      <div style={{ padding: '4px 12px 0', marginTop: 2, display: 'flex', justifyContent: 'space-between', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
        <span style={{ color: DIM, fontSize: 9 }}>KALSHI · {signals.length} SIGNALS</span>
        <span style={{ color: DIM, fontSize: 9 }}>LIVE</span>
      </div>
    </div>
  );
}

// ── Delta Step-Chart (inline) ────────────────────────────────────────────────
function DeltaChart({ signals, selectedDomain }) {
  const signal = signals.find(s =>
    selectedDomain
      ? s.domain?.toUpperCase().includes(selectedDomain.toUpperCase().slice(0, 4))
      : s.oi > 0
  ) ?? signals[0];

  if (!signal) return (
    <div style={{ padding: '20px 12px', color: DIM, fontSize: 9, letterSpacing: '0.15em' }}>AWAITING SIGNAL</div>
  );

  const cur  = signal.signal ?? 50;
  const prev = Math.max(0, cur - (signal.forecast ?? 0));
  const steps = [
    { d: '05-11', v: Math.max(0, prev - 8) },
    { d: '05-12', v: Math.max(0, prev - 4) },
    { d: '05-13', v: Math.max(0, prev) },
    { d: '05-14', v: Math.max(0, prev) },
    { d: '05-15', v: Math.max(0, prev + (cur - prev) * 0.4) },
    { d: '05-16', v: Math.max(0, prev + (cur - prev) * 0.7) },
    { d: '05-17', v: cur },
  ];

  const W = 296, H = 96;
  const PAD = { t: 8, r: 36, b: 20, l: 28 };
  const iW = W - PAD.l - PAD.r;
  const iH = H - PAD.t - PAD.b;
  const maxV = Math.max(...steps.map(s => s.v), 80);
  const xScale = i => PAD.l + (i / (steps.length - 1)) * iW;
  const yScale = v => PAD.t + iH - (v / maxV) * iH;

  const pts = [];
  steps.forEach((s, i) => {
    const x = xScale(i);
    const y = yScale(s.v);
    if (i === 0) { pts.push(`${x},${y}`); return; }
    pts.push(`${x},${yScale(steps[i - 1].v)}`);
    pts.push(`${x},${y}`);
  });

  const lastX = xScale(steps.length - 1);
  const lastY = yScale(cur);

  return (
    <div style={{ padding: '8px 12px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
        <span style={{ color: LIME, fontSize: 8, letterSpacing: '0.2em' }}>DELTA STEP-CHART</span>
        <span style={{ color: DIM, fontSize: 9 }}>{signal.ticker?.slice(0, 12)}</span>
      </div>
      <svg width={W} height={H} style={{ overflow: 'visible' }}>
        {[0, 25, 50, 75].map(v => (
          <g key={v}>
            <line x1={PAD.l - 3} y1={yScale(v)} x2={PAD.l + iW} y2={yScale(v)} stroke="rgba(255,255,255,0.05)" strokeWidth={0.5} />
            <text x={PAD.l - 5} y={yScale(v) + 3} textAnchor="end" fill="rgba(255,255,255,0.2)" fontSize={6}>{v}</text>
          </g>
        ))}
        <line x1={PAD.l} y1={PAD.t + iH} x2={PAD.l + iW} y2={PAD.t + iH} stroke="rgba(255,255,255,0.08)" strokeWidth={0.5} />
        {steps.map((s, i) => (
          <text key={s.d} x={xScale(i)} y={H - 5} textAnchor="middle" fill="rgba(255,255,255,0.18)" fontSize={6}>{s.d}</text>
        ))}
        <polyline fill="none" stroke={LIME} strokeWidth={1.5} strokeLinecap="butt" strokeLinejoin="miter" points={pts.join(' ')} />
        <line x1={PAD.l} y1={lastY} x2={lastX + 4} y2={lastY} stroke="rgba(102,255,0,0.15)" strokeWidth={0.5} strokeDasharray="2,2" />
        <text x={lastX + 6} y={lastY + 3} fill={LIME} fontSize={8} fontFamily={MONO}>{cur}</text>
      </svg>
    </div>
  );
}

// ── Surface Panel ────────────────────────────────────────────────────────────
export default function SurfacePanel({ selectedDomain, onDomainClick }) {
  const [activeTab, setActiveTab]   = useState('TOWERS');
  const [collapsed, setCollapsed]   = useState(false);
  const { signals }                 = useKalshiSignals();

  return (
    <div style={{
      position:        'fixed',
      bottom:          72,
      left:            88,
      zIndex:          12,
      width:           340,
      fontFamily:      MONO,
      pointerEvents:   'auto',
      userSelect:      'none',
    }}>
      {/* Tab strip + collapse toggle */}
      <div style={{
        display:        'flex',
        alignItems:     'center',
        background:     'rgba(0,0,0,0.82)',
        borderTop:      '1px solid rgba(102,255,0,0.18)',
        borderLeft:     '1px solid rgba(102,255,0,0.08)',
        borderRight:    '1px solid rgba(102,255,0,0.08)',
        borderBottom:   collapsed ? '1px solid rgba(102,255,0,0.08)' : 'none',
        backdropFilter: 'blur(12px)',
        WebkitBackdropFilter: 'blur(12px)',
      }}>
        {TABS.map(tab => (
          <button
            key={tab}
            onClick={() => { setActiveTab(tab); if (collapsed) setCollapsed(false); }}
            style={{
              flex:            1,
              background:      'transparent',
              border:          'none',
              borderBottom:    !collapsed && tab === activeTab ? `1px solid ${LIME}` : '1px solid transparent',
              color:           tab === activeTab ? LIME : DIM,
              fontFamily:      MONO,
              fontSize:        8,
              letterSpacing:   '0.18em',
              padding:         '7px 0',
              cursor:          'pointer',
              transition:      'color 150ms',
            }}
          >
            {tab}
          </button>
        ))}
        {/* Collapse toggle */}
        <button
          onClick={() => setCollapsed(c => !c)}
          style={{
            background:    'transparent',
            border:        'none',
            color:         DIM,
            fontFamily:    MONO,
            fontSize:      10,
            padding:       '7px 10px',
            cursor:        'pointer',
            lineHeight:    1,
          }}
        >
          {collapsed ? '▲' : '▼'}
        </button>
      </div>

      {/* Panel body */}
      {!collapsed && (
        <div style={{
          background:     'rgba(0,0,0,0.82)',
          border:         '1px solid rgba(102,255,0,0.08)',
          borderTop:      'none',
          backdropFilter: 'blur(12px)',
          WebkitBackdropFilter: 'blur(12px)',
          maxHeight:      360,
          overflowY:      'auto',
        }}>
          {activeTab === 'TOWERS' && (
            <div style={{ padding: '8px 0' }}>
              <LeverageTowers
                selectedDomain={selectedDomain}
                onDomainClick={onDomainClick}
              />
            </div>
          )}
          {activeTab === 'STACK' && (
            <AttentionStack
              signals={signals}
              selectedDomain={selectedDomain}
              onSignalClick={s => onDomainClick?.(s.domain?.toLowerCase())}
            />
          )}
          {activeTab === 'DELTA' && (
            <DeltaChart
              signals={signals}
              selectedDomain={selectedDomain}
            />
          )}
        </div>
      )}
    </div>
  );
}
