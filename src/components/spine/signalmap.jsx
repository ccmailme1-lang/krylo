...
// WO-275 — Metric Purge: removed nodes count HUD from Signal Map
// WO-276 — Ambient Signal Behavior: atmospheric pulse intensity scales with avgFs
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
        ...
        <span style={{ color: 'rgba(232,244,255,0.4)', fontSize: '9px' }}>{sentimentLabel(node.fs)}</span>
      </div>
...
export default function SignalMap({ data, signalMapData }) {
  ...
  // WO-276 — Keep avgFs ref current for atmospheric pulse scaling
  useEffect(() => {
    if (!activeSignals.length) { avgFsRef.current = 0; return; }
    const sum = activeSignals.reduce((acc, s) => acc + (s.fs ?? 0), 0);
    avgFsRef.current = sum / activeSignals.length;
  }, [activeSignals]);
  ...
  // KRYL-235 + WO-276 — Atmospheric pulse: 4.4Hz, intensity scales with avgFs
  useEffect(() => {
    let frame;
    function tick() {
      if (pulseRef.current) {
        ...
        const fs      = avgFsRef.current;
        const base    = 0.02 + fs * 0.04;
        const swing   = 0.02 + fs * 0.04;
        pulseRef.current.style.opacity = (base + pulse * swing).toFixed(4);
      }
