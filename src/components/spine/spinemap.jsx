...
function Scene({ signals, alertMode, captureRef, lockedIdx, setLockedIdx, selectedSet, setSelectedSet, searchQuery, onRedAlert, heartbeat, onNodeLock, top1Idx, top1Color, top2Idx, top2Color, top3Idx, top3Color, top4Idx, top4Color, top5Idx, top5Color, top6Idx, top6Color, topSixSet, cityMatteSrc, activeCity, leaderboardHoveredIdx, leaderboardClickEvent, onHoverChange, debugMode, activeFilters, onClusterData, activeStateFilter, onStateDoubleClick, onPromotion, lockedZone, setLockedZone, isTimelineMode }) {
  ...
  // WO-746: Leaderboard hover → ConfirmationRing at that node
  useEffect(() => {
    if (leaderboardHoveredIdx == null) return;
    const t0 = performance.now();
    setRingEvents([{ key: `lb-ring-${leaderboardHoveredIdx}-${t0}`, nodeIdx: leaderboardHoveredIdx, t0 }]);
  }, [leaderboardHoveredIdx]);

  // WO-746: Leaderboard click → echo locate ring
  useEffect(() => {
    if (!leaderboardClickEvent) return;
    const { idx, t0 } = leaderboardClickEvent;
    setRingEvents([{ key: `lb-click-${idx}-${t0}`, nodeIdx: idx, t0 }]);
      ...
      if (node.primary) {
        ...
        if (!node.isShattered) {
          ...
          if (isSearchMatch) {
            ...
            // Fire ring event once on anchor
            if (!node._l1RingFired) {
              node._l1RingFired = true;
              const t0 = performance.now();
              setRingEvents(prev => [...prev, { key: `l1-${node.index}-${t0}`, nodeIdx: node.index, t0 }]);
            }
          } else {
  ...
  return (
    <>
      ...
      {debugMode && <>
        <line geometry={zoneRingGeo} position={[0, 0, 0]} scale={[11 * 0.20, 1, 7 * 0.20]}>
          <lineBasicMaterial color="#00ff00" transparent opacity={0.9} depthWrite={false} />
        </line>
        <line geometry={zoneRingGeo} position={[0, 0, 0]} scale={[11 * 0.50, 1, 7 * 0.50]}>
          <lineBasicMaterial color="#ffff00" transparent opacity={0.9} depthWrite={false} />
        </line>
        <line geometry={zoneRingGeo} position={[0, 0, 0]} scale={[11 * 0.78, 1, 7 * 0.78]}>
          <lineBasicMaterial color="#66FF00" transparent opacity={0.9} depthWrite={false} />
        </line>
        <Html position={[-9, 4, 0]} style={{ pointerEvents: 'none' }}>
          <div style={{
            fontFamily: 'IBM Plex Mono, monospace', fontSize: '8px',
            color: '#66ff00', letterSpacing: '0.14em', lineHeight: '1.6',
            background: 'rgba(0,0,0,0.6)', padding: '4px 6px', borderRadius: '2px',
            whiteSpace: 'nowrap',
          }}>
            DEBUG<br/>
            <span style={{ color: '#00ff00' }}>● LOCAL r=0.08</span><br/>
            <span style={{ color: '#ffff00' }}>● REGIONAL r=0.18</span><br/>
            <span style={{ color: '#66FF00' }}>● NATIONAL r=0.33</span>
          </div>
        </Html>
      </>}
...
// ── Export ────────────────────────────────────────────────────────────────────
export default function SignalMap({ data, signalMapData, isActive = false, onSelect }) {
  ...
  return (
    <div
      ref={containerRef}
      style={{ position: 'fixed', inset: 0, zIndex: 5, background: '#000000', pointerEvents: isActive ? 'auto' : 'none' }}
