      ...
      <group ref={spinRef} scale={[FORMATION_SCALE, FORMATION_SCALE, FORMATION_SCALE]}>
        {(() => {
          ...
          return (
            ...
            // time) raced the imperative position write the same way a declarative scale prop
            // once raced the scale write: any re-render (e.g. the nav-surface click) snapped
            // position back to that render's value, independent of the imperative code -- "the
            // whole map drops." Domain groups never unmount (all 6 domains are always created —
            // see coneState above), so there's no first-frame gap where position would sit at
            // an unset default.
