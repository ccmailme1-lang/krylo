...
const NAV_ITEMS = [
  ...
  { id: 'search', label: 'SEARCH',   icon: <svg viewBox="0 0 18 18" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="8" cy="8" r="5"/><line x1="12" y1="12" x2="16" y2="16"/></svg> },
...
const METRICS = [
  ...
  { label: 'IN SEARCH',      value: '028',  delta: '-2 vs last update'  },
...
// ── SHELL LAYOUT ───────────────────────────────────────────────────────────
// pages = array of content components. Only the center rotates.
export default function ShellLayout({ activeNav = 'surface', onNav, onBack, feedSignals = [], pages = [] }) {
    ...
    <div style={{ width: '100%', height: '100%', background: '#000000', display: 'flex', fontFamily: MONO }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          {/* ── CENTER SLOT — carousel. Drop pages here; only this rotates.
               STACKING MODEL (SLCC-001):
                 · pages[pageIndex] = GPU plane (WebGL module — no DOM overlays inside)
                 · TraceryHalo overlay = DOM plane sibling at zIndex:10 (same level as nav arrows)
                 · Nav arrows = zIndex:20
               Overlays MUST live here as siblings, never inside the WebGL module.
               HMR is disabled — Cmd+Shift+R required after every file change. ── */}
          <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', bottom: 16, right: 20, zIndex: 20, display: 'flex', gap: 14, pointerEvents: 'auto' }}>
              <span
                ...
                style={{ fontFamily: BEBAS, fontSize: 18, userSelect: 'none', transition: 'color 0.2s',
                  color: pageIndex > 0 ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.08)',
                  cursor: pageIndex > 0 ? 'pointer' : 'default',
                }}
                onMouseEnter={e => { if (pageIndex > 0) e.currentTarget.style.color = '#ffffff'; }}
                onMouseLeave={e => { if (pageIndex > 0) e.currentTarget.style.color = 'rgba(255,255,255,0.25)'; }}
              >←</span>
              <span
                onClick={() => setPageIndex(i => Math.min(pages.length - 1, i + 1))}
                style={{ fontFamily: BEBAS, fontSize: 18, userSelect: 'none', transition: 'color 0.2s',
                  color: pageIndex < pages.length - 1 ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.08)',
                  cursor: pageIndex < pages.length - 1 ? 'pointer' : 'default',
                }}
                onMouseEnter={e => { if (pageIndex < pages.length - 1) e.currentTarget.style.color = '#ffffff'; }}
                onMouseLeave={e => { if (pageIndex < pages.length - 1) e.currentTarget.style.color = 'rgba(255,255,255,0.25)'; }}
              >→</span>
            </div>
          </div>
        ...
        <div style={{ height: LAYOUT.bottomBarHeight, flexShrink: 0, borderTop: BORDER, display: 'flex', flexDirection: 'column' }}>
          ...
          {/* Section 3 — telemetry */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexShrink: 0, padding: '0 16px', borderTop: DIVIDER, fontSize: 8, letterSpacing: '0.14em', color: 'rgba(255,255,255,0.3)', height: 25 }}>
            {TELEMETRY.map((t, i) => (
