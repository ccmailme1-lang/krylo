...
//
// KRYL-1089 — CORRECTED, 2026-09-13: the line below previously claimed "domain pressure
// store only — no cone dependency (LOCKED 2026-06-20)". That was false. This module reads
// no store, no props, no fetch, anywhere. initSignals() (MOCK SEED, below) seeds a
// hardcoded table; tick() (MOCK OSCILLATOR, below) advances it on Math.random() every
// TICK_MS. Every domain score, every BUILDING/FLAT/DECAYING label, and every hp:* event
// this module emits is generated by a random number generator, not observed signal.
// WO-1826 and WO-1821 are NOT complete against a random source -- both reopened per
// KRYL-1089 (no separate Jira entity exists for either WO-number; this comment is that
// record, since both predate Jira-exclusive ticket numbering).
// DATA SOURCE: none. Mock only. See KRYL-1089 for the fix boundary (disclosure on the
// 5 real consumers, not a real-source swap -- that is separate, unauthorized-here work).
...
// KRYL-1089 — single source of truth for "is this HP output real or simulated." Every
// consumer reads this instead of independently assuming one or the other, so the
// disclosure stays correct automatically if a real source is ever wired in (separate,
// unauthorized-here work) -- no five-file hunt to update when that happens.
export const HP_IS_SIMULATED = true;
