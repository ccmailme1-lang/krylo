...
// WO-258   — Expose prevRecords for decay velocity + momentum derivation
...
export function usetruthlens(query) {
  ...
  // WO-258 — track previous fetch for decay velocity + momentum
  ...
  const fetchTruth = useCallback(async (q) => {
    ...
    try {
      ...
      const enriched = arr.map(enrichRecord);

      // Snapshot current → prev before updating
      setPrevRecords(currentRef.current);
      currentRef.current = enriched;
      setRecords(enriched);
    } catch (err) {
  ...
  return { records, prevRecords, loading, error, refetch: fetchTruth };
}
