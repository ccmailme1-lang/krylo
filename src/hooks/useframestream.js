...
export function useframestream({ enabled = false } = {}) {
    const [latest,   setLatest]   = useState(null);    // most recent decoded signal batch
    const [pressure, setPressure] = useState(null);    // server pressure state
    ...
    return { latest, pressure, lagMs, stats };
}
