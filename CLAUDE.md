...
| 1761  | Domain Coverage Gap — Gig-Income / Alt-Asset (BUG-001A) | BACKLOG — bugs/BUG-005-domain-coverage-gap.md. SPLIT FROM WO-1756. 20/20 batch queries (co-living entry + freelance revenue volatility + DTI/lending stress) routed to Anchor:OPEN/Domain:GENERAL despite specific decision-grade content — no existing domain's keyword set covers this persona shape. Do not hardcode a new domain off this one incident set — re-run a broader corpus and derive taxonomy from actual GENERAL-fallback misses first. Depends on WO-1756 (COMPLETE — corpus re-run needs clean capital extraction first). | YES | NO |
