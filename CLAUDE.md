...
| 1725  | Single-Entity Signal Injection Layer (Musk Protocol) | BACKLOG — FOUNDATIONAL PREREQUISITE for WO-1728 (Bezos/Full-Field Portfolio Convergence). WO-1728 cannot be built until WO-1725 Phase A is complete. Tier planning note: WO-1728 is ranked Tier 2; WO-1725 therefore carries an implicit Tier 2 prerequisite status and must be sequenced before WO-1728 in any build plan. Phase A: entity pressure attribution — tag signals by named entity at ingestion, weight entity contribution per domain. Phase B: cross-domain entity footprint radar/hex overlay (entity pressure share across all 6 cones). Phase C: sentiment manipulation detection gate (MEDIA convergence not corroborated by CAPITAL/OWNERSHIP = flagged). Pass criteria: entity assigned to ≥2 domains shows attributed cone pressure, footprint in BayHUD, Fs≥0.70. Origin: Musk role-play, Fit=9. Depends on WO-1340 (Phase A COMPLETE), WO-1720 (COMPLETE). | YES | NO |
| 1726  | Weak Signal Detection Layer (Webb Protocol) | BACKLOG — Phase A: sub-threshold signal surfacing (pressure < 20) in ATTENTION STACK with distinct visual treatment. Phase B: velocity tracker on sub-threshold signals — fast-rising flagged as emerging. Phase C: cross-domain weak signal correlation — simultaneous rise in TECHNOLOGY + KNOWLEDGE triggers early convergence alert before classifier fires. Pass criteria: weak signals visible below cone pressure floor, early convergence alert fires before BUILDING CONVERGENCE state, Fs≥0.50. Origin: Amy Webb role-play, Fit=8. Depends on WO-1126A (COMPLETE), WO-1336 (COMPLETE). | YES | NO |
...
| 1728  | Full-Field Portfolio Convergence Layer (Bezos Protocol) | BACKLOG — Phase A: simultaneous 6-domain pressure dashboard — when operator spans all 6 domains, surface aggregate cross-domain convergence score alongside individual cone readings. Phase B: domain interdependency map — which cones are causally linked for a given operator profile (e.g. LABOR pressure → CAPITAL pressure for logistics-heavy operators). Phase C: platform inflection alert — when 3+ domains hit BUILDING CONVERGENCE simultaneously, trigger "platform bet window" signal. Pass criteria: 6-domain aggregate score visible, interdependency map renders, platform inflection alert fires at 3+ simultaneous convergences, Fs≥0.70. Origin: Bezos role-play, Fit=8. Depends on WO-1725 (Musk Protocol), WO-1126A (COMPLETE), WO-1336 (COMPLETE). | YES | NO |
...
| 1734  | Non-Consensus Signal Layer (Khosla Protocol) | BACKLOG — Phase A: contrarian gap signal — when KNOWLEDGE cone pressure is rising but CAPITAL cone pressure is absent (delta > 30 points), surface "non-consensus window" classification; the domain looks unfunded but the intellectual signal is building. Phase B: conviction threshold tracker — monitor how long a domain stays in non-consensus state before CAPITAL arrives; historical lead time becomes the conviction window for a thesis-level bet. Phase C: consensus arrival alert — when CAPITAL closes the gap on rising KNOWLEDGE, surface "consensus forming" alert; the non-consensus window is closing. Pass criteria: non-consensus gap fires on KNOWLEDGE/CAPITAL delta > 30, conviction window duration tracked, consensus arrival alert fires when gap narrows below 10, Fs≥0.50. Origin: Vinod Khosla role-play, Fit=8. DEPENDENCY RESOLVED 2026-06-13: WO-1729 reclassified from required → enhancement. Core operation (gap open/gap duration/gap close) requires only current domain pressures — supplied by WO-1126A + WO-1726. WO-1729's 6mo/12mo trend lines and 90-day sustained convergence add Phase C confidence scoring but are not required for any phase to function. Removing WO-1729 as a hard dependency eliminates the Tier-1/Tier-3 conflict. Registry convention established: Required Dependency = WO cannot function without it. Enhancement Dependency = WO functions correctly but gains additional fidelity. Depends on WO-1726 (Webb Protocol — REQUIRED), WO-1126A (COMPLETE — REQUIRED). Enhancement: WO-1729 (long-duration convergence context for Phase C confidence — optional). | YES | NO |
| 1735  | Platform Conviction Arc | BACKLOG — RENAMED from "Platform Formation Signal" (naming collision with WO-1741 META-SIGNAL detector). WO-1735 = interpretation layer (conviction framework for investors/operators). WO-1741 = detection layer (canonical PLATFORM_FORMATION emitter). Arc: Non-Consensus (WO-1734) → Platform Formation signal (WO-1741) → Conviction (WO-1735). Universal VC conviction arc: sequential to WO-1734 (Khosla/non-consensus = early warning); this WO = category validation confirmation. Phase A: formation gate — BUILDING CONVERGENCE on TECHNOLOGY + CAPITAL simultaneously above score 55 triggers "category forming" classification per domain; signals the window when a new investment category has crossed from too-early to fundable. Phase B: velocity qualifier — category formation signal only valid when both TECHNOLOGY and CAPITAL have been rising for ≥14 days (prevents false positives on single-day spikes); rate-of-change tracked per cone. Phase C: hypergrowth overlay — when CAPITAL + LABOR + MEDIA all converge simultaneously (Gil condition), escalate classification from "category forming" to "hypergrowth window"; the scale-stage signal. Pass criteria: formation gate fires on dual convergence > 55, velocity qualifier filters single-day spikes, hypergrowth escalation fires on triple convergence, Fs≥0.70. Serves: Vishria (infrastructure), Leone (global category), Mignot (fintech geography), Gil (growth stage) — 4 Midas List 7s reading the same signal. Sequential to WO-1734: Khosla enters on non-consensus gap; these four enter when this gate fires. Depends on WO-1734 (Non-Consensus Layer), WO-1126A (COMPLETE), WO-1336 (COMPLETE), WO-1719 (FRED — COMPLETE). | YES | NO |
| 1736  | Regulatory Convergence Window (Gass-Benecke Protocol) | BACKLOG — Derived from AI/ML legal practitioners (Andy Gass, Latham & Watkins; Danielle Benecke, Baker McKenzie). Phase A: regulation precursor signal — when KNOWLEDGE + MEDIA converge simultaneously above score 50 on a TECHNOLOGY domain, surface "regulatory window forming" classification; signals 6–18 months before legislation reaches draft stage. Phase B: jurisdiction velocity tracker — rate-of-change on MEDIA pressure across geographic signals; when MEDIA accelerates in multiple regions simultaneously, cross-border regulatory alignment is imminent. Phase C: enforcement posture gate — when KNOWLEDGE pressure exceeds CAPITAL pressure in a TECHNOLOGY domain by >20 points, surface "enforcement ahead of investment" alert; signals regulatory risk is outpacing market adoption. Pass criteria: regulatory window fires on KNOWLEDGE + MEDIA dual convergence > 50, jurisdiction velocity tracks multi-region acceleration, enforcement gate fires on KNOWLEDGE/CAPITAL spread > 20, Fs≥0.50. Serves AI & tech lawyers timing client compliance posture before legislation lands. Depends on WO-1126A (COMPLETE), WO-1336 (COMPLETE), WO-1733 (Godin/Attention Protocol). | YES | NO |
| 1737  | HNW Client Convergence Overlay (Cornerstone Protocol) | BACKLOG — Derived from Cornerstone Group Silicon Valley (Morgan Stanley, Menlo Park) serving tech executives, VC/PE partners, and founders. Phase A: client portfolio convergence signal — TECHNOLOGY + CAPITAL + OWNERSHIP simultaneous convergence triggers "portfolio timing window" classification; surfaces the macro moment when tech-adjacent HNW clients should rebalance. Phase B: liquidity event gate — OWNERSHIP cone EDGAR Form D spike + CAPITAL cone compression = impending liquidity event signal (IPO window closing or private placement surge); actionable for concentrated position management. Phase C: sector rotation alert — when TECHNOLOGY pressure peaks and CAPITAL begins declining (divergence > 15 points), surface "sector rotation imminent" for tech-concentrated portfolios. Pass criteria: portfolio timing window fires on triple convergence, liquidity event gate fires on EDGAR spike + CAPITAL compression, rotation alert fires on divergence > 15, Fs≥0.70. Depends on WO-1720 (EDGAR — COMPLETE), WO-1719 (FRED — COMPLETE), WO-1735 (Platform Conviction Arc), WO-1126A (COMPLETE). | YES | NO |
| 1738  | Critical Materials Demand Signal (Lacaze Protocol) | BACKLOG — Derived from Amanda Lacaze, CEO Lynas Rare Earths (Australia), world's only major rare earth producer outside China. Phase A: supply chain reorientation signal — when TECHNOLOGY + CAPITAL + OWNERSHIP converge simultaneously on a materials domain, surface "Western supply chain repositioning" classification; the window when governments and manufacturers are accelerating non-China sourcing decisions. Phase B: geopolitical pressure gate — when MEDIA + OWNERSHIP pressure both exceed 60, surface "geopolitical supply risk" alert; signals impending policy action (export controls, subsidies, emergency procurement) before it reaches legislation. Phase C: demand pipeline signal — EDGAR Form D deal flow filtered for mining, materials, and defense manufacturing; when commitments accelerate, rare earth demand follows 12–24 months later. Pass criteria: repositioning signal fires on triple convergence, geopolitical gate fires on dual threshold > 60, demand pipeline tracks EDGAR commitments, Fs≥0.70. Depends on WO-1720 (EDGAR — COMPLETE), WO-1719 (FRED — COMPLETE), WO-1126A (COMPLETE). | YES | NO |
...
| 1741  | Platform Formation Signal | BACKLOG — CLASS: META-SIGNAL / DETECTION. Identifies when an emerging category has progressed beyond experimentation into ecosystem formation. Trigger: Convergence State = BUILDING AND TECHNOLOGY AND CAPITAL simultaneously above score 55. Output: PLATFORM_FORMATION. Surface copy examples: "Technology and capital alignment increasing. Category formation probability rising." / "Infrastructure adoption crossing viability threshold." Consumer protocols: Satya Nadella (partnership/acquisition window), Eric Vishria (enterprise infrastructure bet), Andy Jassy (cloud adoption wave), Douglas Leone (global category formation), Martin Mignot (fintech expansion window). WO-1731 (Collison) covers fintech specifically — this WO is the universal pattern. Velocity qualifier v2: both domains must be rising for ≥14 days (prevents single-day spike false positives). Depends on WO-1126A (COMPLETE), WO-1336 (COMPLETE), WO-1735 (Platform Conviction Arc — persona interpretation layer). | YES | NO |
