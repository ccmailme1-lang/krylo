...
OPEN WORK ORDERS — BUILD SEQUENCE (updated 2026-06-17)
Start at 1776. Work backwards. This is the only open list.
...
| 1769  | Refinitiv / High-Freq Feed Procurement | BACKLOG — specs/WO-1769-refinitiv-feed-procurement.md. Vendor contract + S3 landing zone + 30-day POC. Unblocks WO-1770 → WO-1771 (VWSC Phase B). No code until contract signed. Origin: WO-1768 phase split. | YES | NO |
| 1768-A | Macro Timing Proxy v1 | BACKLOG (APPROVED) — specs/WO-1768-A-macro-timing-proxy.md. Three-layer: Fs*=BAMLH0A0HYM2÷M2V (raw, threshold 8.5) + DFC=EDGAR totalOfferingAmount concentration (top-2 sectors ≥55%=HIGH) + YCID=consecutive T10Y2Y inversion days (≥30=THETA_DRAG). Reconcile: FADE_SIGNAL/MAXIMUM on all three; FADE_SIGNAL/MEDIUM on Fs*+DFC; PASS otherwise. API: GET /v1/timing-proxy on port 4000. YCID persisted in runtime/ycid_state.json. Raw FRED fetch independent of useFredSignals.js. NBER recession markers for back-test. Zod validation. 10-day sprint. BAU: qa_wo1768a_timing_proxy.mjs. | YES | NO |
| 1768  | INVESTOR Synthesizer — Macro Divergence Signal Layer | COMPLETE — src/engine/querysynthesis.js. synthInvestor(): 4 branches: isMacro (CROWDED TRADE DETECTED / STRUCTURAL DIVERGENCE with fade flag), isPortfolio (PORTFOLIO PRESSURE), isTactical (TACTICAL SETUP ACTIVE), default (INVESTOR SIGNAL ACTIVE). Intent classification via regex. Confidence scored from query specificity + capital presence. ncHits → deRatio → classifyLeverageTier(). Full output contract: stateLabel/confidence/primaryInsight/momentum/trajPoints/attentionStack/keyDrivers/recommendedAction/timeHorizon/impactLevel/bluf/purpose/fiveWs/evidence/assumptions/assessment/threats/opportunities/alternativeView/outlook/actions/leverage. INVESTOR removed from synthGeneral fallback. Build clean. WO-1768-A (macro timing proxy) = BACKLOG — activates only during macro-stress regimes, Phase B enhancement. | YES | YES |
| 1766  | Domain Ambiguity Gate | COMPLETE — src/engine/domainambiguitygate.js. classifyAmbiguity(domainScores) → {state,entropy,winner,winnerScore,coActive,resolutionEligible}. HARD_MARGIN=0.35, MIN_CONFIDENCE=0.25, SOFT_BAND=0.35, ENTROPY_HARD=1.0, ENTROPY_HOLD=2.2. Shannon H + margin + min-confidence triple gate. BAU: qa_wo1766_ambiguity_gate.mjs. 8/8 PASS. | YES | YES |
| 1767  | Weighted Domain Superposition Layer | COMPLETE — src/engine/querysynthesis.js. detectDomain() now returns DomainVector {primary,weights,state,entropy,coActive,resolutionEligible}. synthesizerFor(vector) added. resolvePrimary() extracted (compound priority rules preserved). DOMAIN_SCORE_PATTERNS + scoreDomains() for co-activity scoring. synthesizeQuery() updated: queryDomain=vector.primary (string, backward compat), domainVector added to output. classifyAmbiguity imported from domainambiguitygate.js. Protected domain gate wraps as synthetic HARD vector. HOLD falls back to synthGeneral (v1 — UI stability). BAU: qa_wo1767_superposition.mjs. 11/11 PASS. Build clean. | YES | YES |
| 1776  | Operational Carry Risk Modeler (Beast Industries Protocol) | BACKLOG — specs/WO-1776-operational-carry-risk.md. Stress-test layer on top of WO-1775 DEP field. Three risk vectors: (1) Personality Impairment Risk — Donaldson moat collapse (reputational event → DEP → ~0); (2) Governance Transition Friction — move-fast creator culture → corporate-compliance friction quantified as governance_readiness_score; (3) Operational Chokepoint Risk — Virtual Dining Concepts-class disputes that collapse brand equity faster than financial fundamentals. Output: CARRY_RISK_STATE (LOW/ELEVATED/CRITICAL), dep_at_risk, governance_readiness_score, chokepoint_exposure. Architecture: CarryRisk = DEP × reputational_volatility. BLOCKED until WO-1775 ships — cannot stress-test a baseline that does not exist. Origin: MrBeast/Beast Industries role-play, Fit=8. Depends on WO-1775. | YES | NO |
| 1775  | Creator HoldCo Synthesizer (Beast Industries Protocol) | BACKLOG — specs/WO-1775-creator-holdco-synth.md. synthCreatorHoldco() in querysynthesis.js. Maps INVESTOR lens queries containing creator/holdco/flywheel/personality/donaldson/mrbeast/beast signals. Key variables: DEP (Donaldson Equity Premium — personality-as-structural-asset premium, bidirectional scalar), FV (Flywheel Velocity = media views / retail conversion rate), PM (Platform Multiplier = CPM arbitrage from Creator Platform), GD (Governance Discount = IPO-readiness friction). Architecture requirement: DEP must be bidirectional — WO-1776 inverts it (CarryRisk = DEP × reputational_volatility) without a contract change. Valuation frame: Creator HoldCo multiple = Flywheel Premium above Disney/NBCU baseline. Four branches: isFlywheel (FLYWHEEL ACCELERATION / FLYWHEEL STALL), isValuation (CREATOR_HOLDCO PREMIUM), isGovernance (IPO READINESS FRICTION), default (CREATOR HOLDCO SIGNAL ACTIVE). Depends on WO-1768 (synthInvestor — COMPLETE), WO-1767 (domain superposition — COMPLETE), WO-1720 (EDGAR — COMPLETE), WO-1126A (COMPLETE). Origin: MrBeast/Beast Industries role-play, Fit=8. | YES | NO |
