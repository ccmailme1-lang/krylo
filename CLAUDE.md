...
| 1761  | Domain Coverage Gap — Gig-Income / Alt-Asset (BUG-001A) | SCOPED, not built — specs/WO-1761-domain-coverage-gap.md + bugs/BUG-005-domain-coverage-gap.md. 30-query diversity corpus (.batchtest3.mjs) run per this WO's own prerequisite. Resolves to 3 distinct problems: (a) true coverage gaps, 17/25 queries, no domain matches at all — needs a real taxonomy decision, Founder input required, not a one-off keyword patch; (b) substring/keyword contamination — CONFIRMED bug, not a coverage gap: detectDomain()'s AUTO regex has no word boundary, matches "car" inside "card"/"cards", routed a credit-card-debt query to the car-buying synthesizer ("dealer network," "MSRP" advice). Cheap mechanical fix, no taxonomy judgment, can ship independently; (c) content-model mismatch on CORRECT routing — e.g. "mortgage" in a reverse-mortgage query correctly hits REAL_ESTATE but that synthesizer assumes a forward purchase loan, and with no plausibility floor on price/down (unlike AUTO's >=1000 filter) produced confirmed broken output: "$68 purchase, $480,000 down (705882%), Financed: $-479,932". Harder problem, needs its own design pass. Depends on WO-1756 (COMPLETE). | YES | NO |
