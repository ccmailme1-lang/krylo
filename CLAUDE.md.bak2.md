⚠️ ZERO GUESSING ALLOWED. NON-NEGOTIABLE.
If you do not know the exact value, position, size, or coordinate — STOP and ask. Never approximate, estimate, or infer visual or spatial values. Measure from the source or request the information explicitly.

CLAUDE.md — KRYLO Agent Operating Procedures
Version: 2.4 | Status: LOCKED | Last Updated: 2026-05-22
...
UPDATED: 2026-05-14
    ...
    Layer 1: Hero (Entry)
        ...
        Implementation: Exclusively public/krylo2-feed.html.
...
4. FORENSIC GUARDRAILS (ANTI-DRIFT)
    ...
    THE BRIDGE RULE: Communication between Layer 0/1 (krylo2-feed.html) and the React Engine happens exclusively via postMessage. The listener lives in src/app.jsx and watches for type: 'krylo-submit'.
...
In 2026, the best approach to website typography is purposeful contrast — not purely consistent (same font everywhere) nor chaotic (too many fonts). Text is partitioned by authority.
    ...
    The Impact Voice (Layer 0 Display) — LOCKED 2026-05-03:
        ...
        Usage: All large display headlines in public/krylo2-feed.html.
        Locked values:
...
| 1307  | Boundary Gate Attenuation        | PENDING — ground threshold rings on cone footprints. Breach triggers stroke pattern change. src/components/spine/conemap.jsx. | NO   | NO         |
| 1308  | Historical Flux Trajectory       | PENDING — Ghost Cones. Cached prior-tick wireframe silhouette offset behind live cone. Shares buffer with WO-1003. src/components/spine/conemap.jsx. | NO   | NO         |
| 1309  | Dynamic Frontier Waveforms       | PENDING — per-frame vertex noise on cone rings driven by volatility. Zero-allocation Float32Array reuse. src/components/spine/conemap.jsx. | NO   | NO         |
| 1310  | Driver Node-Map Overlay          | PENDING — Inspection panel flips to 2D directed graph on cone click. Maps cone → root entities. D3 or React Flow overlay. | NO   | NO         |
| 1311  | Ingestion Horizon                | PENDING — Bottom 120px live sparklines from FlowController SSE. #66FF00 phosphor. Replaces static ticker. | NO   | NO         |
| 1312  | Momentum Leaderboard             | PENDING — Volatility Index tab in inspection panel. React list sorted by 24hr Δ across domain signals. | NO   | NO         |
| 1313  | Geospatial Topology Toggle       | PENDING — ABSTRACT/TOPOLOGY switch. Concentric grid fades, wireframe map fades in. Cone coords lerp to geographic/sector anchors via THREE.Vector3.lerp. | NO   | NO         |
