# SPEC — Structural Field Map (PROPOSED — mathematically-enhanced revision)

**Status:** PRESERVED, NOT ADOPTED. This is Founder-supplied material, relocated here from
`New Simulator Graphic.md` (a Cognitive Fabric placeholder file it should not have overwritten —
Founder 2026-09-02).
**Domain:** KRYL-1243 / Structural Field Map — the 6-domain radial polygon on the North-South
Simulation Panel (`structuralfield.jsx`). **NOT a Cognitive Fabric artifact.**
**Evaluation owed:** against the shipped `SPEC-structural-field-map.md` + `structuralfield.jsx`
(shipped `5e54623` geometry / `dd38a68` label-fit / `7fd4f14` tooltip; `qa_structuralfield.mjs`
21/21). Per `feedback_falsification_not_design_mode` — a proposed revision to a shipped, closed
model gets *tested against the live implementation*, not adopted. Not on the CF review path; does
not touch the CF hold, branch, or any gate.

**Two Founder corrections attached to this proposal (not yet merged into the text):**
1. Pipeline order: `… → OBSERVATION → EVIDENCE → RELATIONSHIP ADMISSION → FORMATION` (the
   original text put EVIDENCE downstream of RELATIONSHIP — inconsistent with the KRYLO contract;
   evidence is upstream). The Structural Field must stay downstream of the already-produced field
   state and never become an alternate path to evidence or formation.
2. Single canonical polygon order: *"The canonical polygon order is the fixed domain order
   defined by the authoritative angular assignment. No alternate ordering is permitted."* Remove
   the "or any fixed cyclic order" alternative from §3.
