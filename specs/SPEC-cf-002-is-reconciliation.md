...
| **1 render wiring** | **DONE (Founder-authorized 2026-09-02, commit `96cc387`, branch only).** `daemon.js`: `startCFProducer({tickMs:null})` at start + `cfProducerTick()` in `runCycle()`'s `finally` (one CF pass per 30s ingestion cycle, guarded) + `stopCFProducer()` on stop — daemon = cadence source, not data source. `targetpacket.jsx`: `<CFField />` as a distinct section immediately after `05 PROVENANCE`. Validated: 3 synchronous `inferFormation(field.particles)` call sites byte-identical; no render path imports an analytical CF module; `cffield.jsx` imports only `cf/read.js`; no `.jsx` self-timer; full CF harness suite green (5). Also aliased `pathwaystore.js` to `CANONICAL_DOMAINS` (−1 pre-existing ontology-guard violation). NOTE: `npm run build` prebuild ontology guard was already RED on HEAD before Gate 1 (4 non-CF files + the frozen `cfpathwaystore.js`) — Gate 1 introduced zero build regressions; that's a separate ticket. |
| **3 real-I/O telemetry** | **UNBLOCKED.** Amended criterion: measured telemetry MUST correspond to **actual connector activity** (real `dispatchBatch` volume, real pool sizes, pathway `source` ≠ `'fixture'`), not the harness path. Gate 3 results return for a separate production-integration decision. |
